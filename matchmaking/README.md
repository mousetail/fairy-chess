...
as the socket opens; it carries the protocol version and the levels this server
knows about, so a client can adapt rather than guess.

### Client → server

| Message       | Fields                                                                | Meaning                                                                                               |
| ------------- | --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `join`        | `complexity: number`, `name?: string`                                 | Ask to be matched. Re-sending it while waiting updates the preference without losing your place.      |
...
| Message          | Fields                                                                            | Meaning                                                                                                                             |
| ---------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `welcome`        | `protocolVersion`, `minComplexity`, `maxComplexity`, `complexityLabels: string[]` | Sent once on connection.                                                                                                            |
| `queued`         | `complexity`, `waiting`                                                           | You are in the queue, and this many players are in it.                                                                              |
...
{"type":"matched","gameId":"game-1","color":"white","opponentName":"Bob",
 "complexity":2,"complexityLabel":"several fairy pieces","board":{"pieces":[…],"turn":"white","halfTurnNumber":0,"symbols":{…}}}
...
The server is the authority. It picks the chaos level, generates the position
with `ChessGame.defaultLayout`, and will only relay a move that matches one its
own `getValidMoves` produces for the piece, on the board it is holding.
...
`deno task test` covers the matching policy, the referee, the lobby, the
serialisation and the message parser, and finishes with two clients playing a
real game over real WebSockets. The tests that open sockets bind to `127.0.0.1`
on a port the operating system picks, so they need `--allow-net`.
